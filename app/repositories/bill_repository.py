...
class BillRepository(GenericRepository):
    ...
    def find_bills_by_user_id(self, user_id: int, cache_model: Optional[CacheModel] = None) -> list:
        """
        ...
        try:
            ...
            bills = session.query(self.model).filter(
            ...
            logger.error("Error finding bills by user_id: %s",
                         error, exc_info=True)
            return []
        finally:
            session.close()
    ...
    def _to_dict(self, instance: Bill) -> dict:
        ...
        return {
            ...
            "total_amount": float(instance.total_amount) if instance.total_amount is not None else None,
        }
